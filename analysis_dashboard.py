...
def configure_fy_window(df) -> list:
    ...
    Raises ValueError if the data has fewer than 2 fiscal years (trend fitting
    ...
    if len(fy_cols) < 2:
        raise ValueError(
            f"Need at least 2 fiscal years for trend analysis; found {fy_cols}"
        )
...
def pivot_by_year(df, group_col, value_col="Loans (In House + Not In House)"):
    ...
    trend_data = pv[FY_COLS].apply(
        lambda row: pd.Series(_fit_trend(row.values),
                              index=["Trend Slope (loans/yr)",
                                     "Mean Annual Loans",
                                     "Trend R^2"]),
        axis=1,
    )
    pv = pd.concat([pv, trend_data], axis=1)
